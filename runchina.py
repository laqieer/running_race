        ...
        for result in results:
            if result["raceDistance"] == raceDistances[raceItem]:
                if not has_result:
                    has_result = True
                    print(f"\n# {raceItem}\n")
                    print("|Race|Date|Race Level|Gun Time|Chip Time|Split Time|")
                    print("|---|---|---|---|---|---|")
                print(f"|{result['raceName']}|{result['scoreTime']}|{result['competitionType']}|{result['scoreShot']}|{result['scoreChip']}|{'<br>'.join([x['paragraphName'] + '<br>' + x['paragraphScore'] for x in result['paragraphScoreList']])}|")
